...
def collect_data():                            #функция, скачивающая данные с заданного количества страниц
    ...
    with open("routes.txt","w", encoding="utf-8") as database:    #открываем запись в файл
        ...
        while True:                                         #цикл по страницам
            ...
            for name in names:                              #бежим по столбцам дата-таблицы
                ...
                print(
                    cells["RouteNumber"], #получает номер маршрута
                    cells["TrackOfFollowing"],  #получает цепочку остановок маршрута
                    *cells["geoData"]["coordinates"][0], #получает цепочку координат маршрута
                    (str(z)+"\n"), #кольцевой или нет                
                    sep=";", file=database
                    )             
