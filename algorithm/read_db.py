...
def TEST_PRINT(graph, routes, sqlite_connection, seq_stops, seq_routes):
    try:
        cur = sqlite_connection.cursor()
...
def sequence_id(sqlite_connection, name_table):
    try:
        cur = sqlite_connection.cursor()

        sql_command = "SELECT _id FROM "
        sql_command += name_table
...
def main():
    try:
        ...
        seq_stops = sequence_id(sqlite_connection, "stopsker")
        seq_routes = sequence_id(sqlite_connection, "routesker")

    except Exception as e:
        print({e})
        exit()
    else:
        TEST_PRINT(graph, routes, sqlite_connection, seq_stops, seq_routes)
    finally:
        sqlite_connection.close()
